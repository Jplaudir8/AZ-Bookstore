<template>
  <div id="confirmationView">
    <div id="card">
      <img id="confirmation-check" src="@/assets/images/confirmation-check.png">
      <h1>Thank you for your order!</h1>

      <p class="top-info">Confirmation #: {{ $store.state.orderDetails.order.confirmationNumber }}</p>
      <p class="top-info">Time: {{ transactionDateFormat(new Date($store.state.orderDetails.order.dateCreated)) }}</p>
      ...
      <div id="customerInfo">
        ...
        <p>{{ formatPhoneNumber($store.state.orderDetails.customer.phone) }}</p>
        <p>**** **** **** {{ retrieveLast4Digits($store.state.orderDetails.customer.ccNumber) }} ({{ ccExpDateFormat(new Date($store.state.orderDetails.customer.ccExpDate)) }})</p>
      </div>
      ...
      <div id="total-section">
        <div id="totals">
          <div class="totals-child">
            <p>Subtotal:</p>
            <p>{{ $store.state.orderDetails.order.amount - $store.state.cart.surcharge | asDollarsAndCents }}</p>
          </div>
          <div class="totals-child">
            <p>Surcharge:</p>
          ...
          <div class="totals-child bold-font">
            <p>Total:</p>
            <p>{{ $store.state.orderDetails.order.amount | asDollarsAndCents }}</p>
          </div>
        </div>
      </div>
  </div>
